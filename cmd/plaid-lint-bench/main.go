...
func main() {
	...
	var (
		...
		cascadeFile     = flag.String("cascade-file", "", "file to touch for the cascade scenario (overrides the bench_cascade default). The default cascade edit appends a comment trailer to this file and does NOT revert it; use --cascade-restore-on-exit to defend against accidental edits to non-disposable paths (e.g. a real repository worktree).")
		...
		allowCWMismatch = flag.Bool("allow-cold-warm-digest-mismatch", false, "Phase 1.7 sub-path-c C.4: demote the cold↔warm diagnostic-digest equivalence assertion from a hard error to a flag in BenchmarkResult.cold_warm_digest_mismatch. Required for large-monorepo sweeps because the cold+warm+cascade+leaf shape surfaces a pre-existing at-scale determinism issue (cold and warm produce different diagnostic digests at N=0, with no clustering involved). Default false preserves the hot-path hard-stop contract.")
	...
	// Honor SIGINT/SIGTERM so a long monorepo-scale run can be aborted cleanly.
...
// fixtures don't care; a real repository worktree strongly does.
