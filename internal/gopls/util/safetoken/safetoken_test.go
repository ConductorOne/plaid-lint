...
func TestGoplsSourceDoesNotCallTokenFileMethods(t *testing.T) {
	...
	for _, pkg := range pkgs {
		switch pkg.PkgPath {
		case "go/token",
			"github.com/conductorone/plaid-lint/internal/gopls/util/safetoken", // this package
			"github.com/conductorone/plaid-lint/internal/gopls/cache/parsego":  // copies go/parser/resolver.go
