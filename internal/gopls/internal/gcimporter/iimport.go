...
import (
	...
	"github.com/conductorone/plaid-lint/internal/gopls/internal/aliases"
...
// uint64Count reads a length-prefix count that will be used to size an
// allocation, and rejects any count that cannot possibly be backed by the
// remaining input. Every counted element occupies at least one byte of
// input (a varint is >= 1 byte; a manifest entry is several), so a count
// larger than the number of unread bytes is provably malformed.
//
// This guards against a hostile or corrupt count prefix — e.g. from a
// poisoned shared cache entry — that would otherwise commit gigabytes via
// make() before the per-element read loop ever fails. recover() cannot
// catch a kernel OOM kill, so the count must be validated before the
// allocation, not after.
func (r *intReader) uint64Count() uint64 {
	n := r.uint64()
	if n > uint64(r.Len()) {
		errorf("import %q: length prefix %d exceeds %d remaining bytes", r.path, n, r.Len())
	}
	return n
}
...
func iimportCommon(fset *token.FileSet, getPackages GetPackagesFunc, data []byte, bundle bool, path string, shallow bool, reportf ReportFunc) (pkgs []*types.Package, err error) {
	...
	if shallow {
		...
		fLen = int64(r.uint64())
		fileOffset = make([]uint64, r.uint64Count())
	...
	items := make([]GetPackagesItem, r.uint64Count())
	...
	// Exclude concurrent readers (facts.Set.Encode → objectpath.For, analyzer
	// Runs walking shared *types.Package scopes/types) for the duration of the
	// scope-and-type writes. The atomic unit must span the entire decl loop
	// because r.obj() constructs *types.Named with nil .underlying via
	// r.declare(), and only later sets .underlying via SetUnderlying — a reader
	// that races in between sees a nil-underlying Named and panics inside
	// objectpath.finder.find.
	//
	// Reentrance: this lock spans only scope/type writes. The earlier
	// getPackages(items) call (line 278) may have recursively re-entered
	// iimportCommon for transitive packages — that recursion ran with the lock
	// not yet held (or already released by the inner call), so we cannot
	// deadlock by acquiring now. Within this loop, p.doDecl(transitive_pkg,
	// name) only Lookups (already-complete transitive packages short-circuit),
	// so no reentrant Lock is attempted.
	typesyncmu.Lock()
	defer typesyncmu.Unlock()
...
func (p *iimporter) doDecl(pkg *types.Package, name string) {
	...
	// No explicit lock: doDecl runs inside iimportCommon's decl loop which
	// holds typesyncmu.Lock for the entire run.
...
func (r *importReader) declare(obj types.Object) {
	// No explicit lock: declare runs inside iimportCommon's decl loop which
	// holds typesyncmu.Lock for the entire run. The lock spans declare AND the
	// subsequent SetUnderlying / SetTypeParams calls — see iimportCommon.
...
func (r *importReader) doType(base *types.Named) (res types.Type) {
	...
	case aliasType, definedType:
		...
		r.p.doDecl(pkg, name)
		// No lock needed here: this code path is only reachable from inside
		// iimportCommon's decl loop, which holds typesyncmu.Lock. See the
		// outer comment in iimportCommon.
