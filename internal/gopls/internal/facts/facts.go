...
func (d *Decoder) Decode(read func(pkgPath string) ([]byte, error)) (*Set, error) {
	return d.DecodeFromGobFacts(func(pkgPath string) (GobFacts, error) {
		data, err := read(pkgPath)
		if err != nil {
			return GobFacts{}, err
		}
		return DecodeGobFacts(data)
	})
}

// GobFacts is the raw (post-gob-decode) representation of one
// dependency's serialized fact set. Callers that hold the same
// Set.Encode output across many sibling consumers can share the cost
// of gob.Decode by performing it exactly once per blob. Zero value
// is an empty fact set.
type GobFacts struct {
	facts []gobFact
}

// Len reports the number of facts.
func (g GobFacts) Len() int { return len(g.facts) }

// DecodeGobFacts gob-decodes a serialized facts blob produced by
// [Set.Encode]. Empty blob is a valid encoding of an empty fact set.
func DecodeGobFacts(data []byte) (GobFacts, error) {
	if len(data) == 0 {
		return GobFacts{}, nil
	}
	if hook := decodeCountHook.Load(); hook != nil {
		(*hook)()
	}
	var gf []gobFact
	...
	return GobFacts{facts: gf}, nil
}

// DecodeFromGobFacts is [Decoder.Decode] but with the gob decode
// hoisted out so callers can memoize it across consumers. An empty
// GobFacts from read is equivalent to an empty blob.
func (d *Decoder) DecodeFromGobFacts(read func(pkgPath string) (GobFacts, error)) (*Set, error) {
	...
	for _, imp := range d.pkg.Imports() {
		...
		logf("decoded %d facts: %v", len(gf.facts), gf.facts)
...
// decodeCountHook, when non-nil, fires on every non-empty
// DecodeGobFacts call. Test-only.
var decodeCountHook atomic.Pointer[func()]

// SetDecodeCountHook installs (or clears, when fn == nil) the
// DecodeGobFacts counter hook. Returns a restore function. Test-only.
func SetDecodeCountHook(fn func()) func() {
	var ptr *func()
	if fn != nil {
		ptr = &fn
	}
	prev := decodeCountHook.Swap(ptr)
	return func() { decodeCountHook.Store(prev) }
}
...
// EncodeAndGobFacts returns [Set.Encode]'s bytes plus the pre-gob-
// decoded slice the encoder built internally. The returned GobFacts
// aliases that slice and must NOT be mutated.
func (s *Set) EncodeAndGobFacts() ([]byte, GobFacts) {
	b, gf := s.encode()
	return b, GobFacts{facts: gf}
}

// encode is the shared implementation of Encode and EncodeAndGobFacts.
func (s *Set) encode() ([]byte, []gobFact) {
	...
	// objectpath.For. RWMutex permits all encoders to run in parallel — the race
	// is only against importers, not against other readers.
	// See typesyncmu.
	rtok := typesyncmu.RLock()
	defer typesyncmu.RUnlock(rtok)
	...
	return buf.Bytes(), gobFacts
}

// IsWellFormed reports whether b is a syntactically valid encoded
// facts.Set blob (the output of Set.Encode). The check is purely
// structural — it does NOT require a *types.Package or attempt to
// reconstitute objectpath references — so it can be used by callers
// that hold the blob but no longer have the originating package
// (e.g. plaid-lint's L1 cache validation path, W6).
//
// Returns true for an empty blob (len(b) == 0), since Set.Encode
// returns an empty byte slice when the package produced no facts.
func IsWellFormed(b []byte) bool {
	if len(b) == 0 {
		return true
	}
	var gobFacts []gobFact
	return gob.NewDecoder(bytes.NewReader(b)).Decode(&gobFacts) == nil
}
