...
func (s *Snapshot) config(ctx context.Context, allowNetwork AllowNetwork) *packages.Config {
	cfg := &packages.Config{
		...
		// Tests defaults to true (matches golangci-lint v2's
		// `run.tests` default); a non-nil Options.Tests == false
		// opts out so *_test.go files are skipped.
		Tests: s.view.folder.Options.Tests == nil || *s.view.folder.Options.Tests,
	}
	if !allowNetwork {
