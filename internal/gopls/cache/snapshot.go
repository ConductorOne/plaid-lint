...
import (
	...
	"github.com/conductorone/plaid-lint/internal/gopls/cache/testfuncs"
	"github.com/conductorone/plaid-lint/internal/gopls/file"
	"github.com/conductorone/plaid-lint/internal/gopls/filecache"
	label1 "github.com/conductorone/plaid-lint/internal/gopls/label"
	...
	"github.com/conductorone/plaid-lint/internal/gopls/util/constraints"
	...
	"github.com/conductorone/plaid-lint/internal/gopls/util/pathutil"
	...
	"github.com/conductorone/plaid-lint/internal/gopls/internal/event/label"
...
type Snapshot struct {
	...
	// l0DepOverrides is the dep-override map. When non-nil it is
	// copied into each typeCheckBatch this snapshot constructs (see
	// acquireTypeChecking). Entries are synthetic analyzeSummary values
	// the engine installed from L0 hits before calling Analyze; an
	// analysisNode whose PackageID has an override skips its action
	// graph entirely.
	l0DepOverrides map[PackageID]*analyzeSummary

	// onNodeAnalyzed is the post-analysis callback. When non-nil
	// the analysis driver invokes it once per node (in enqueue, after
	// the summary is set onto the node but before decrefPreds may nil
	// it) so the engine can capture per-analyzer fact blobs and write
	// L0 entries for both root and dep packages.
...
func (s *Snapshot) clone(ctx, bgCtx context.Context, changed StateChange, done func()) (*Snapshot, bool) {
	...
	needsDiagnosis := len(changed.CompilerOptDetails) > 0 || len(changed.ModuleUpgrades) > 0
