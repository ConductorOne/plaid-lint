...
import (
	...
	"github.com/conductorone/plaid-lint/internal/gopls/bloom"
	"github.com/conductorone/plaid-lint/internal/gopls/cache/metadata"
	"github.com/conductorone/plaid-lint/internal/gopls/cache/parsego"
	...
	"github.com/conductorone/plaid-lint/internal/gopls/internal/packagesinternal"
	"github.com/conductorone/plaid-lint/internal/gopls/internal/typesinternal"
	"github.com/conductorone/plaid-lint/internal/gopls/internal/typesyncmu"
	"github.com/conductorone/plaid-lint/internal/gopls/internal/versions"
	...
	"github.com/conductorone/plaid-lint/internal/gopls/util/tokeninternal"
...
type typeCheckBatch struct {
	...
	// filecache for dep-side export data. See W5.
	l2          *clcache.Cache
	l2BuildEnv  string
	l2GoVersion string
	l2ToolVer   string
	...
	// l1 is the optional plaid-lint L1 cache (set when the owning View
	// was constructed against a Cache that had AttachL1 called). When
	// non-nil, each (analyzer, package) action consults it ahead of
	// running the analyzer body. See W6.
	l1         *clcache.Cache
	l1ToolVer  string
	l1Metrics  *l1Metrics
	l1Registry *analyzers.Registry // per-analyzer descriptors; nil → analyzers.BundledRegistry

	// irManager is the optional L3 coordination interface.
	// When non-nil, action.exec pins the analyzer's package before
	// invoking a NeedsIR=true analyzer's Run body and releases the
	// pin once Run returns. Plumbed from the owning Cache via
	// AttachIRManager. Nil means "no L3 coordination" — the W7
	// default.
	irManager l3.IRManager

	// scheduler is the optional W9 coordinator. When non-nil,
	// action.exec consults
	// scheduler.Acquire before running the analyzer body and
	// defers the returned release function. The default (nil)
	// preserves the W7/W8 byte-equivalent path. The W9
	// production attachment is a *scheduler.RSSBudgetScheduler;
	// the type is held as an interface here so cache and
	// scheduler avoid a circular import.
	scheduler ActionScheduler

	// l0Overrides is the dep-override map. When non-nil and a key
	// for an analysisNode's package is present, analysisNode.runCached
	// short-circuits the action graph entirely and returns the
	// synthetic summary directly. The override is installed by the
	// engine before calling Snapshot.Analyze (via
	// Snapshot.SetL0DepOverrides); each entry is built from an L0 hit's
	// diagnostic + per-analyzer fact stream.
	//
	// Per-synth Actions release. Each value in
	// l0Overrides is a *analyzeSummary. After a node consumes its
	// override and the L0 onNodeAnalyzed callback aliases the Facts
	// out by value, releaseL0OverrideActions clears the synth's
	// Actions map (the last persistent reference to the gob-decoded
	// fact blobs). The map itself is set once before Analyze and read
	// concurrently from enqueue bodies; mutation happens only on the
	// per-id *analyzeSummary value, never the map header.
	l0Overrides map[PackageID]*analyzeSummary

	// l0OverridesReleased tracks which override entries have already had
	// their Actions map cleared by releaseL0OverrideActions. Using a
	// sync.Map (rather than a guarded plain map) keeps the reader path
	// in l0OverrideFor lock-free; the per-id CAS ensures the
	// synth.Actions clear runs exactly once even under concurrent
	// release calls from multiple enqueue goroutines.
	l0OverridesReleased sync.Map // PackageID → struct{}

	// onNodeAnalyzed is the post-analysis callback installed by
	// the engine. Invoked in enqueue after each node's summary is set
	// and before decrefPreds() may free it. Nil disables the callback.
	onNodeAnalyzed func(L0NodeData)

	// l1CanonicalCache caches the (URI → PkgPath, PkgPath → dir) pair
	// used to canonicalize / resolve gobDiagnostic URIs at L1 write /
	// read. Built lazily on first use from b._handles; see
	// l1CanonicalMaps. Nil until first access.
	l1CanonicalCacheMu sync.Mutex
	l1CanonicalCache   *l1CanonicalCacheT

	// canonicalFsetCache memoizes the canonical clone of b.fset
	// produced by canonicalBatchFileSet. M2c eliminates the
	// per-L2-write rebuild storm.
	//
	// Invalidation key is (b.fset.Base(), canonicalFsetVersion).
	// Base() grows on AddFile (gcimporter, direct parser.ParseFile).
	// parseCache.parseFiles uses AddExistingFiles with pre-allocated
	// bases below b.fset's starting Base (reservedForParsing), so
	// Base() does not move on those additions; checkPackage bumps
	// canonicalFsetVersion to cover that path.
	canonicalFsetVersion      atomic.Int64
	canonicalFsetMu           sync.Mutex
	canonicalFsetCache        *token.FileSet
	canonicalFsetCacheBase    int
	canonicalFsetCacheVersion int64

	// loadPhase is the typecheck-before-analyze barrier. Non-nil
	// only when PLAID_INPUT_DIGEST=1 is set AND the batch was created
	// for a Snapshot.Analyze invocation. Nil for callers that don't
	// dispatch through the analyze enqueue (forEachPackage and friends).
	// Installed by Snapshot.Analyze before the first node enqueue.
	loadPhase *loadPhaseBarrier

	// loadPhaseClaimedByBarrier records whether a loadPhaseBarrier has
	// taken ownership of the typesyncmu load-phase counter claim that
	// acquireTypeChecking installed. When true, the matching
	// ExitLoadPhase is the barrier's responsibility (fired either in
	// closeNow on barrier close or in releaseAtTeardown on dispatch
	// abort). When false, acquireTypeChecking's teardown branch fires
	// the matching ExitLoadPhase. Atomic so the teardown read is safe
	// without holding the batch's typeCheckMu.
	loadPhaseClaimedByBarrier atomic.Bool

	// sharedBuildir is the M1 workspace-scoped *ir.Program cache,
	// allocated lazily on the first buildir dispatch when
	// PLAID_SHARED_BUILDIR=1. Lifetime is the batch; the program is
	// GC-eligible once the batch tears down. Nil when the M1 flag is
	// off or no buildir analyzer has run yet. See shared_buildir.go.
	sharedBuildirOnce sync.Once
	sharedBuildir     *workspaceBuildir
}

// canonicalBatchFileSet returns a clone of b.fset whose *token.File
// names are canonical plaid-lint paths (rather than absolute
// filesystem paths). Every Pos valid in b.fset at call time is valid
// (and resolves to the same line/column) in the returned FileSet; the
// filename component differs. The clone is memoized on the batch; see
// canonicalFsetCache for the invalidation key (M2c).
func (b *typeCheckBatch) canonicalBatchFileSet() *token.FileSet {
	b.canonicalFsetMu.Lock()
	defer b.canonicalFsetMu.Unlock()
	base := b.fset.Base()
	version := b.canonicalFsetVersion.Load()
	if b.canonicalFsetCache != nil &&
		b.canonicalFsetCacheBase == base &&
		b.canonicalFsetCacheVersion == version {
		return b.canonicalFsetCache
	}
	uriPkg, _ := b.l1CanonicalMaps()
	fresh := canonicalizeFileSet(b.fset, uriPkg)
	b.canonicalFsetCache = fresh
	b.canonicalFsetCacheBase = base
	b.canonicalFsetCacheVersion = version
	return fresh
}
...
func (s *Snapshot) acquireTypeChecking() (*typeCheckBatch, func()) {
	...
	if s.batch == nil {
		...
		// Bracket the typecheck batch as a load phase. Every
		// typesyncmu.Lock writer (gcimporter declare; clcache.Read/
		// EncodeExportData) only fires while a batch is alive, so the
		// counter is non-zero whenever a writer can be in flight.
		// RLock readers see counter > 0 and acquire mu.RLock as
		// before, preserving the original RWMutex semantics.
		//
		// Under PLAID_INPUT_DIGEST=1, Snapshot.Analyze
		// installs a loadPhaseBarrier that takes ownership of this
		// claim and early-closes the load phase between typecheck and
		// analyze. The handoff is recorded on the batch
		// (loadPhaseClaimedByBarrier); when set, the teardown branch
		// below skips its ExitLoadPhase call to avoid double-release.
		typesyncmu.EnterLoadPhase()
		// Plumb the optional L2 cache from the owning View's Cache.
		if c := s.view.c; c != nil && c.l2 != nil {
			s.batch.l2 = c.l2
			s.batch.l2BuildEnv = c.l2BuildEnv
			s.batch.l2GoVersion = c.l2GoVersion
			s.batch.l2ToolVer = c.l2ToolVer
			s.batch.l2Metrics = &c.l2Metrics
		}
		// Plumb the optional L1 cache from the owning View's Cache.
		if c := s.view.c; c != nil && c.l1 != nil {
			s.batch.l1 = c.l1
			s.batch.l1ToolVer = c.l1ToolVer
			s.batch.l1Metrics = &c.l1Metrics
			if c.l1Registry != nil {
				s.batch.l1Registry = c.l1Registry
			} else {
				s.batch.l1Registry = analyzers.BundledRegistry
			}
		}
		// Plumb the optional L3 IRManager from the owning View's Cache.
		// Nil is preserved as "no coordination" — action.exec treats
		// nil as a NoopIRManager.
		if c := s.view.c; c != nil && c.irManager != nil {
			s.batch.irManager = c.irManager
		}
		// Plumb the optional W9 ActionScheduler.
		if c := s.view.c; c != nil && c.scheduler != nil {
			s.batch.scheduler = c.scheduler
		}
		// Plumb the optional L0 dep-override map from the snapshot.
		// The engine installs it (via SetL0DepOverrides) before invoking
		// Snapshot.Analyze, so the map is stable for the duration of the
		// batch. The batch keeps its own pointer so concurrent
		// acquireTypeChecking calls don't race on the snapshot field.
		if len(s.l0DepOverrides) > 0 {
			s.batch.l0Overrides = s.l0DepOverrides
		}
		// Plumb the onNodeAnalyzed callback if installed.
		if s.onNodeAnalyzed != nil {
			s.batch.onNodeAnalyzed = s.onNodeAnalyzed
		}
	}
	s.batchRef++
	...
	return s.batch, func() {
		...
		if s.batchRef == 0 {
			...
			//
			// When a loadPhaseBarrier has claimed ownership of
			// the load-phase counter (Snapshot.Analyze under flag=1),
			// the barrier's releaseAtTeardown is responsible for the
			// matching ExitLoadPhase. The flag is set by Snapshot.
			// Analyze and read here under typeCheckMu.
			if !batch.loadPhaseClaimedByBarrier.Load() {
...
//
// If id is in the syntax-package set for this batch, we route
// to b.getPackage so the import view shares *types.Package identity with
// the syntax type-check. The pre-fix path created a separate
// L2-decoded or import-only *types.Package, which the type checker would
// then reject in assignments against the syntax-checked version with
...
func (b *typeCheckBatch) getImportPackage(ctx context.Context, id PackageID) (pkg *types.Package, err error) {
	return b.importPackages.get(ctx, id, func(ctx context.Context) (*types.Package, error) {
		...
		// W5: plaid-lint L2 fast path. The L2 entry stores a shallow
		// gcimporter blob keyed by ph.key. On hit we hand the
		// caller a *types.Package without consulting filecache or
		// re-type-checking; on miss tryL2WriteThrough type-checks ph
		// for import, writes the resulting shallow blob to L2, and
		// returns the fresh pkg.
		//
		// Cross-flow identity: every *types.Package returned
		// from this futureCache is canonical for ph.mp.ID. Shallow
		// L2 decodes route cross-pkg references through
		// b.getImportPackage (the enclosing futureCache.get), so a
		// reader that needs ph.mp.PkgPath's *types.Package gets the
		// same instance this body returns.
		if b.l2 != nil {
			if pkg, ok := b.tryL2Lookup(ctx, ph); ok {
				return pkg, nil
			}
			return b.tryL2WriteThrough(ctx, ph)
		}
...
// l2ActionID builds the L2 action ID for the package referenced by ph.
// ph.key carries the gopls reachability hash, which already folds in
// the package's source files plus the transitive dep keys. We feed it
// into L2Entry.InputDigest; DepTypeDigest is zero because ph.key
// already includes the dep contribution.
func (b *typeCheckBatch) l2ActionID(ph *packageHandle) clcache.ActionID {
	e := &clcache.L2Entry{
		PackageID:   string(ph.mp.ID),
		GoVersion:   b.l2GoVersion,
		BuildEnv:    b.l2BuildEnv,
		ToolVersion: b.l2ToolVer,
	}
	// ph.key is file.Hash ([32]byte); copy its bytes into InputDigest.
	copy(e.InputDigest[:], ph.key[:])
	return clcache.ComputeL2ActionID(e)
}

// tryL2Lookup attempts to read and decode the L2 entry for ph. On
// success it returns the rehydrated *types.Package; on any failure
// (miss, decode error, or shallow-import error) it returns ok=false
// and the caller falls through to gopls's existing path. The function
// records hit / miss / error counters on b.l2Metrics.
//
// The L2 blob is in shallow format. IImportShallow's
// getPackages callback routes every cross-package reference through
// b.getImportPackage — the same futureCache used by every other import
// path in this batch — so the decoded *types.Package shares identity
// with the canonical instance for each transitively-referenced path.
// Positions land in b.fset; the canonical fileData
// table embedded in the shallow blob keeps cross-machine portability.
func (b *typeCheckBatch) tryL2Lookup(ctx context.Context, ph *packageHandle) (*types.Package, bool) {
	if b.l2 == nil {
		return nil, false
	}
	id := b.l2ActionID(ph)
	entry, err := b.l2.ReadL2(id)
	if err != nil {
		if b.l2Metrics != nil {
			b.l2Metrics.misses.Add(1)
		}
		return nil, false
	}
	if len(entry.ExportData) == 0 {
		if b.l2Metrics != nil {
			b.l2Metrics.errors.Add(1)
		}
		return nil, false
	}
	// IImportShallow takes typesyncmu internally (iimportCommon at
	// iimport.go:338) so we MUST NOT acquire it here — that would
	// deadlock. The per-acquire hold time inside IImportShallow is
	// bounded by ph.mp's local decl count: the shallow blob encodes
	// only this package's Scope; cross-pkg references resolve
	// read-only via objectpath.Object inside the decoder, which is
	// the load-phase contention shrink the shallow format trades for.
	getPackages := b.l2GetPackagesCallback(ctx, ph.mp)
	pkg, err := gcimporter.IImportShallow(b.fset, getPackages, entry.ExportData, string(ph.mp.PkgPath), nil)
	if err != nil {
		if b.l2Metrics != nil {
			b.l2Metrics.errors.Add(1)
		}
		// On a decode error the fall-through path will recompute the
		// export data; we do NOT propagate the error to the caller
		// because L2 is an opportunistic optimization.
		return nil, false
	}
	if b.l2Metrics != nil {
		b.l2Metrics.hits.Add(1)
	}
	return pkg, true
}

// l2GetPackagesCallback builds a gcimporter.GetPackagesFunc that
// resolves every transitive package referenced by mp's shallow blob to
// the canonical *types.Package held by b.importPackages. Self-references
// (item.Path == mp.PkgPath) get a fresh package that the decoder will
// populate; every other path is resolved by b.getImportPackage, which
// is also what importPackage's existing callback (check.go:1051) uses.
// Routing through the same futureCache is what preserves
// cross-flow identity under shallow mode.
//
// The self-reference Package is created with item.Name (the name
// encoded into the blob's manifest), not mp.Name: gcimporter's
// IImportShallow asserts pkg.Name() == item.Name and panics
// otherwise, and metadata.Package.Name can be empty for synthetic
// handles. item.Name is authoritative — it was written by
// IExportShallow from pkg.Name() at encode time.
func (b *typeCheckBatch) l2GetPackagesCallback(ctx context.Context, mp *metadata.Package) gcimporter.GetPackagesFunc {
	lookup := importLookup(mp, b)
	return func(items []gcimporter.GetPackagesItem) error {
		for i, item := range items {
			if item.Path == string(mp.PkgPath) {
				items[i].Pkg = types.NewPackage(item.Path, item.Name)
				continue
			}
			id, alt := lookup(PackagePath(item.Path))
			if alt != "" {
				return fmt.Errorf("inconsistent metadata during L2 import: for package path %q, found both IDs %q and %q", item.Path, id, alt)
			}
...
// tryL2WriteThrough type-checks ph for import, stores the resulting
// pkg as a shallow blob in L2 (best-effort), and returns the fresh
// pkg. The canonical *types.Package per path is held by
// b.importPackages (the futureCache enclosing this call), so any
// concurrent reference to ph.mp.PkgPath — direct or via the shallow
// getPackages callback — resolves to this pkg.
func (b *typeCheckBatch) tryL2WriteThrough(ctx context.Context, ph *packageHandle) (*types.Package, error) {
	if ph.mp.PkgPath == "unsafe" {
		return types.Unsafe, nil
	}
	freshPkg, err := b.checkPackageForImport(ctx, ph)
	if err != nil {
		return nil, err
	}
	// Best-effort write to L2 so subsequent lookups hit cleanly.
	// l2StoreWithFiles uses IExportShallow under the hood;
	// passing nil files keeps the existing per-pkg FileSet snapshot
	// behaviour unchanged (the in-process read path doesn't consult
	// FileSetSnapshot — positions are decoded into b.fset directly
	// via the shallow blob's embedded fileData table).
	if b.l2 != nil {
		b.l2StoreWithFiles(ph, freshPkg, nil)
	}
	return freshPkg, nil
}

// l2Store writes an L2 entry for p using the just-computed package.
// Errors are recorded and swallowed: the L2 write is opportunistic.
//
// The FileSetSnapshot section of the entry carries a per-package
// *token.FileSet built from p's own *token.File entries, preserving the
// original file bases used in b.fset so positions remain consistent
// with the encoded export data. Encoding b.fset directly produced
// O(packages²) on-disk growth on large closures.
func (b *typeCheckBatch) l2Store(ph *packageHandle, p *Package) {
	if b.l2 == nil || p == nil || p.pkg == nil || p.pkg.types == nil || ph == nil {
		return
	}
	files := make([]*token.File, 0, len(p.pkg.compiledGoFiles)+len(p.pkg.goFiles))
	for _, pgf := range p.pkg.compiledGoFiles {
		if pgf != nil && pgf.Tok != nil {
			files = append(files, pgf.Tok)
		}
	}
	...
	b.l2StoreWithFiles(ph, p.pkg.types, files)
}

// l2StoreWithFiles writes an L2 entry for pkg, using files as the
// source for the per-package FileSetSnapshot. files must be the
// *token.File entries owned by pkg (registered against b.fset). It is
// safe to pass duplicates: l2StoreWithFiles dedupes by pointer
// identity. See l2Store and perPackageFileSet for context.
//
// Skip-on-present: the L2 ID is derived from (ph.key, PackageID,
// GoVersion, BuildEnv, ToolVersion) — the same inputs that produced
// any prior entry at this ID. If a file already exists at the target
// path the write is elided; bytes-on-disk are content-addressed by
// construction.
func (b *typeCheckBatch) l2StoreWithFiles(ph *packageHandle, pkg *types.Package, files []*token.File) {
	if b.l2 == nil || pkg == nil || ph == nil {
		return
	}
	// Cheap skip path: the L2 ID is derivable from the metadata header
	// fields alone (without ExportData/FileSetSnapshot bytes). When the
	// on-disk file already exists, skip the expensive
	// gcexportdata.Write + file-set encode + atomic-rename. Mirrors the
	// CAS-already-present skip that filecache.Set already does (its
	// O_TRUNC-avoidance check) for the L1-adjacent typecheck cache.
	headerID := b.l2ActionID(ph)
	if b.l2.HasL2(headerID) {
		if b.l2Metrics != nil {
			b.l2Metrics.skipped.Add(1)
		}
		return
	}
	// IExportShallow reads pkg.Scope() via Scope.Names + Scope.Lookup;
	// guard against concurrent gcimporter Scope.Insert writers.
	//
	// Hand the encoder a canonical clone of the batch-wide
	// FileSet whose *token.File entries carry canonical
	// "<pkgPath>/<basename>" names. The canonical names end up inside
	// the shallow blob's per-pkg fileData table rather than the
	// absolute paths b.fset carries, preserving the cross-machine
	// portability of L2 ActionIDs.
	canonBatchFset := b.canonicalBatchFileSet()
	rtok := typesyncmu.RLock()
	exportData, err := gcimporter.IExportShallow(canonBatchFset, pkg, nil)
	...
	uriPkg, _ := b.l1CanonicalMaps()
	perPkgFset := canonicalizeFileSet(perPackageFileSet(files), uriPkg)
	fsetData, err := clcache.EncodeFileSet(perPkgFset)
	if err != nil {
		// FileSet encoding failure is unusual but not fatal: store
		// without the FileSet snapshot. Cross-process consumers won't
		// be able to recover positions exactly, but the in-process
		// gcexportdata.Read path doesn't need it (positions are
		// reconstructed into the consumer's FileSet directly).
		fsetData = nil
		if b.l2Metrics != nil {
			b.l2Metrics.errors.Add(1)
		}
	...
	if b.l2Metrics != nil {
		b.l2Metrics.stores.Add(1)
	}
}

// perPackageFileSet builds a fresh *token.FileSet containing only the
// supplied *token.File entries, preserving the original Base of each
// file so any token.Pos encoded against the original (batch-wide)
// FileSet still resolves to the same file in the per-package FileSet.
//
// Used by l2StoreWithFiles to bound the size of the FileSetSnapshot
// section of each L2 entry.
func perPackageFileSet(in []*token.File) *token.FileSet {
	// Dedupe by pointer identity (a single *token.File can appear in
	// both compiledGoFiles and goFiles), then sort by Base —
	// AddExistingFiles requires non-overlapping inputs sorted by Base.
	// Both invariants are satisfied for files registered against a
	// single token.FileSet (b.fset), so dedup + sort is sufficient.
	seen := make(map[*token.File]struct{}, len(in))
	files := make([]*token.File, 0, len(in))
	for _, tf := range in {
		if tf == nil {
			continue
		}
	...
	return tokeninternal.FileSetFor(files...)
}
...
func (b *typeCheckBatch) getPackage(ctx context.Context, ph *packageHandle) (*Package, error) {
	return b.syntaxPackages.get(ctx, ph.mp.ID, func(ctx context.Context) (*Package, error) {
		...
		if ph.state >= validImports {
			...
			for depPath, depID := range ph.mp.DepsByPkgPath {
				g.Go(func() error {
					...
					if err == nil {
						...
						impMu.Lock()
						importErrs = append(importErrs, importErr{depID: depID, err: err})
						impMu.Unlock()
					}
					return err
				})
			...
			if len(importErrs) > 0 && os.Getenv("PLAID_ADR11_TRACE") == "1" {
				adr11TraceImportFailures(ph.mp.ID, importErrs)
			}
		}
...
func storePackageResults(ctx context.Context, b *typeCheckBatch, ph *packageHandle, p *Package) {
	...
	if p.metadata.PkgPath != "unsafe" { // unsafe cannot be exported
		...
		// W5: also publish a standard gcexportdata blob to the L2 cache
		// when one is attached. This populates the dep-side cache that
		// later imports will hit via tryL2Lookup. Failures are recorded
		// in l2Metrics but never propagated.
		if b != nil {
			b.l2Store(ph, p)
		}
	}
...
func (b *typeCheckBatch) checkPackageForImport(ctx context.Context, ph *packageHandle) (*types.Package, error) {
	...
	go func() {
		...
		// W5: mirror the in-process filecache write into the L2 cache
		// so dep-side imports computed via checkPackageForImport are
		// also visible to future runs that consult L2 first.
		impFiles := make([]*token.File, 0, len(pgfs))
		for _, pgf := range pgfs {
			if pgf != nil && pgf.Tok != nil {
				impFiles = append(impFiles, pgf.Tok)
			}
		}
		b.l2StoreWithFiles(ph, pkg, impFiles)
	}()
	return pkg, nil
}
...
func localPackageKey(inputs *typeCheckInputs) file.Hash {
	...
	// file names and contents — fold canonical "<pkgPath>/<basename>"
	// in place of the absolute file URI inside file.Identity.
	// The hash content (file.Identity.Hash) is unchanged; only the
	// URI string is rewritten. This keeps two machines that check out
	// the same source at different absolute paths producing
	// byte-identical localKeys (and therefore byte-identical L1/L2
	// ActionIDs through ph.localKey / ph.key).
	pkgPath := string(inputs.pkgPath)
	fmt.Fprintf(hasher, "compiledGoFiles: %d\n", len(inputs.compiledGoFiles))
	for _, fh := range inputs.compiledGoFiles {
		writeCanonicalIdentity(hasher, fh, pkgPath)
	}
...
func (b *typeCheckBatch) checkPackage(ctx context.Context, fset *token.FileSet, ph *packageHandle, imports map[PackagePath]*types.Package) (*Package, error) {
	...
	// M2c: parseCache.parseFiles into b.fset uses AddExistingFiles with
	// pre-allocated bases below b.fset's starting Base, so Base() does
	// not move; bump the memo's version stamp explicitly.
	if pkg.fset == b.fset {
		b.canonicalFsetVersion.Add(1)
	}
