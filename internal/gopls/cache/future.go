...
// release drops the cache entry for k, returning the computed value (or
// the zero value if the entry was absent or unfinished). This is the
// aggressive-release escape hatch used by [analysisNode.releaseAggressive]
// to evict a persistent entry once no in-flight consumer needs it.
//
// release does not cancel any in-flight computation for k; it only
// removes the persistent reference from the cache map so the value can
// be garbage collected once any concurrent caller (already inside a get
// closure on this key) returns.
func (c *futureCache[K, V]) release(k K) (V, bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
	f, ok := c.cache[k]
	if !ok {
	...
	delete(c.cache, k)
	// If the computation completed, hand the value back so the caller
	// can clear any aliasing references it might hold; otherwise return
	// the zero value (the in-flight computation will publish into a
	// local copy of f that no future get will reach).
	select {
	case <-f.done:
		v := f.v
		var zeroV V
		f.v = zeroV // drop the cache's reference to the value itself
		return v, true
	default:
		var zero V
		return zero, false
